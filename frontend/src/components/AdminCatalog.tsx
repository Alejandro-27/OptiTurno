...
export default function AdminCatalog() {
  ...
  const filteredServices = services.filter((s) => {
    ...
    if (activeFilter === "pausado")
      return matchesSearch && s.status === "Pausado";
          ...
          <div className="flex items-center gap-3 self-end md:self-auto">
            <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-lg border border-slate-200 dark:border-slate-800">
              ...
              <button
                ...
                className={`px-3 py-1 text-[11px] font-bold rounded transition-colors ${activeFilter === "activo" ? "bg-white dark:bg-indigo-600/15 text-indigo-600 dark:text-indigo-400 shadow-sm dark:shadow-none" : "text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200"}`}
              >
                Activos
              </button>
              <button
                onClick={() => setActiveFilter("pausado")}
                className={`px-3 py-1 text-[11px] font-bold rounded transition-colors ${activeFilter === "pausado" ? "bg-white dark:bg-indigo-600/15 text-indigo-600 dark:text-indigo-400 shadow-sm dark:shadow-none" : "text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200"}`}
              >
                Pausados
              </button>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg px-4 py-2 font-semibold text-xs transition-all shadow-md shadow-indigo-600/15 active:scale-95"
            >
              <Plus size={14} />
              Agregar Servicio
            </button>
