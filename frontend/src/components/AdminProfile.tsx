...
export default function AdminProfile() {
  ...
  const [description, setDescription] = useState(
    "SaaS vanguardista de estilismo premium, ofreciendo cortes exclusivos y tratamientos de cuidado facial de alta gama.",
  );
      ...
      <div className="bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm dark:shadow-xl transition-colors duration-200">
        <div className="flex justify-between items-center max-w-xl mx-auto mb-4">
          <div className="flex flex-col items-center">
            ...
            <span className="text-[9px] uppercase font-bold text-slate-500 dark:text-slate-400 mt-1.5">
              Información
            </span>
          </div>
      ...
      {activeStep === 1 && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">
                Teléfono de Soporte (WhatsApp compatible)
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400 dark:text-slate-500 pointer-events-none">
                  <Phone size={14} />
          ...
          <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800/80">
            <button
              type="submit"
              className="group flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg px-6 py-2.5 font-bold text-xs uppercase tracking-wider transition-all shadow-md shadow-indigo-600/15 active:scale-95"
            >
              Siguiente Paso
              <ArrowRight
                size={14}
      ...
      {activeStep === 2 && (
        ...
        >
          <div className="flex items-center gap-1.5 border-b border-slate-100 dark:border-slate-800 pb-3">
            ...
            />
            <h4 className="text-sm font-bold text-slate-900 dark:text-slate-200">
              Personalización Visual del Portal Cliente
            </h4>
          </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 flex flex-col justify-center">
                  <span className="text-xs font-bold text-slate-900 dark:text-slate-200">
                    Previsualización del Isotopo
                  </span>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                    Sugerido: Formato PNG transparente de 512px por 512px para
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl flex gap-3">
              ...
              <div>
                <span className="text-xs font-bold text-slate-900 dark:text-slate-200 block">
                  Canal SSL de Seguridad
                </span>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 leading-relaxed">
                  Todos los datos generados por sus clientes y transacciones
                  financieras cumplen con el estándar PCI-DSS. OptiTurno no
                  comparte información sensible con terceros.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-between pt-4 border-t border-slate-100 dark:border-slate-800/80">
            <button
              ...
              className="py-2.5 px-6 border border-slate-300 dark:border-slate-700 bg-transparent hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors"
            >
              Atrás
            </button>
            <button
              type="submit"
              disabled={isLoading}
              className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg px-6 py-2.5 font-bold text-xs uppercase tracking-wider transition-all shadow-md shadow-indigo-600/15 disabled:opacity-50 active:scale-95"
            >
              {isLoading && (
        ...
        <div className="bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800 rounded-xl p-8 shadow-sm dark:shadow-xl text-center space-y-6 animate-scale-up transition-colors duration-200">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 max-w-md mx-auto leading-relaxed">
              Tu software OptiTurno ya se encuentra activo en producción. El
              subdominio privado ahora está abierto para recibir citas de
              clientes.
            </p>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-950/80 p-4 rounded-xl max-w-sm mx-auto border border-slate-200 dark:border-slate-800/80">
            ...
            <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400 font-mono mt-1.5 block">
              https://{subdomain}.optiturno.com
            </span>
          </div>

          <div className="flex justify-center gap-4 pt-4 border-t border-slate-100 dark:border-slate-800/60 max-w-md mx-auto">
            <button
              type="button"
              onClick={() => setActiveStep(1)}
              className="flex-1 py-2 px-4 border border-slate-300 dark:border-slate-700 bg-transparent hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors"
            >
              Editar Perfil
            </button>
            <button
