...
export const sembrarDatosInicialesService = async () => {
  ...
  const { data: sucursal, error: errS } = await supabase
    .from('sucursales')
    .insert([{ negocio_id: negocio.id, nombre: 'Sede Central Anapoima', direccion: 'Calle 4 #5-12', telefono: '3101234567' }])
    .select()
    .single();
  if (errS) throw errS;
  ...
  const { data: servicios, error: errSer } = await supabase
    .from('servicios')
    .insert([
      { sucursal_id: sucursal.id, nombre: 'Corte de Cabello Premium', descripcion: 'Incluye lavado y perfilado de cejas', precio: 25000.00, duracion_minutos: 30 },
      { sucursal_id: sucursal.id, nombre: 'Barba Esculpida y Toalla Caliente', descripcion: 'Ritual tradicional con navaja', precio: 18000.00, duracion_minutos: 30 },
      { sucursal_id: sucursal.id, nombre: 'Combo Rey (Corte + Barba)', descripcion: 'El servicio completo de la casa', precio: 38000.00, duracion_minutos: 60 }
    ])
    .select();
